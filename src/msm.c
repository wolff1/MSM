...
*/
	...
	InnerRange.Ranges = (GRID_RANGE_MIN_MAX*) dynvec((Msm->prm.p+1)*(Msm->prm.p+1), sizeof(GRID_RANGE_MIN_MAX));	//	FIXME - this is too big
	...
	for (m = 0; m < OuterRange.NumSlices; m++)
	{
		for (i = OuterRange.Ranges[m].Min; i <= OuterRange.Ranges[m].Max; i++)
		{
			...
			(*FineGrid->get_grid_points_coarse)(FineGrid, i, CoarseGrid, &InnerRange);
			for (n = 0; n < InnerRange.NumSlices; n++)
			{
//printf("Slice %ld, (%ld,%ld)\n", n, InnerRange.Ranges[n].Min, InnerRange.Ranges[n].Max);
...
//printf("(i,j) = (%ld,%ld)\n", i,j);
//					(*CoarseGrid->increment_grid_point_value)(CoarseGrid, j, Val);
					(*FineGrid->increment_grid_point_value)(FineGrid, i, Val);
				}
			}
		}
	}
